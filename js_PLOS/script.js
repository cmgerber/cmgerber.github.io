...
//load data for collapsing tree
  ...
  d3.json("data_PLOS/plos_tree.json", function(error, plos) {
    root = plos;
    root.x0 = height / 2;
    root.y0 = 0;
...
d3.json("data_PLOS/dict_subj_leaf.json", function(error, leaf) {
  if (error) return console.warn(error);

  leaf_look = leaf;
});

d3.json("data_PLOS/dict_subj_top.json", function(error, top) {
  if (error) return console.warn(error);

  top_look = top;
});

d3.json("data_PLOS/dict_subject_dec.json", function(error, dec) {
  if (error) return console.warn(error);
...
d3.json("data_PLOS/articles_coded.json", function(error, json) {
  if (error) return console.warn(error);
